...
One value of group_by queries is to look for combinations that should never happen and get a sense of how inconsistent your data is. Try creating a query that: 

* Counts the murders by relationship. That is, how many are there that say "Wife", "Husband", etc. Keep both the relationship_code and the relationship variables in your query. 

* Filter just for husbands and wives in the relationship, then add the gender of the victim and offender to your query.  

You should see that there are some errors in the coding in the original data. IRL, you'd have to figure out what to do with that.
...
#### Add in sex of victim and offender {-}

Filter using either the relationship_code or the relationship. You can use the %in% operator for this. Try arranging by the most number of cases to the least

```{r}

murder_data %>%
  filter ( relationship %in% c("Husband", "Wife")) %>%
  group_by (relationship, vic_sex, off_sex) %>%
  summarise ( num_of_cases = n() ) %>%
