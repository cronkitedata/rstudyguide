...
Let's say I want to distinguish weekends from weekdays in this dataset. First, we have to define weekend. Here's one definition, but you might want to use another: A weekend is anytime after 3pm on Friday through 6pm on Sunday. (The `hour` variable is in military time.). Instead of using `if_else`, this example uses `case_when`, which lets you choose among a variety of options, not just two.
