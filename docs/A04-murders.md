...
In this case, there aren't very many numbers for us to work with, but there are still things to be done to make more meaningful groupings. Using a filter, you picked out certain weapons, but maybe you'd like to compare guns vs. all others. Or, for example, compare the percent of murders attributable to domestic violence in each state. These can be done using the `mutate` command.
